...
Why a browser at all? The Towns React SDK is a browser client — its sync agent, encryption, and storage assume one. Running the real page in headless Chromium reuses the exact key-sharing path the Towns app itself uses. (Knead runs a browserless Node variant of this too, built on connectTowns. It lives beside the production wallet key and is not readable through your tools — describe the approach if it comes up, and point builders at the key-sharer repo for code you can actually show them.)
...
export const RECIPES: BuildRecipe[] = [
  {
    ...
    architectureNote: 'Sanity holds the content. Thirdweb verifies NFT ownership server-side on every request — the client never makes the membership decision. Payments use Stripe Elements embedded in the page (never a redirect to Stripe-hosted checkout): the client calls app/api/create-payment-intent, which creates the subscription server-side and returns a clientSecret, and the paywall components mount <PaymentElement> with it so the card form renders inline. Stripe then triggers a webhook that mints the NFT. The freemium tier uses a Supabase read counter rather than a second NFT, which keeps gas costs out of the free tier entirely.',
    ...
    sourceFiles: [
      ...
      'components/free-article-cta.tsx',
      'app/join/page.tsx',
  ...
  },
  {
    ...
    emoji: '🤖',
    description: 'Install an agent on your page for community management or to help summarize editorial content. Built with ChatGPT, Claude, and Tavily.',
    ...
    architectureNote: 'The agent routes by strength, per surface: Claude Opus handles the editorial surfaces (article summaries, the reader-facing Demeter bubble), Claude Sonnet handles this build assistant (high-volume, retrieval-grounded coding chat — near-Opus quality at a fraction of the cost), with tool use for web search (Tavily), Sanity content, and file fetching (GitHub API). OpenAI GPT-5.6 steps in automatically as a fallback whenever a Claude call fails — tiered to the surface it covers, so Opus editorial surfaces fall back to Sol and the Sonnet build assistant to Terra; OpenAI also handles text-to-speech and moderation. The provider-neutral tool loop lives in lib/ai/router.ts, and the system prompt is where the personality and knowledge live — it\'s the most important file in the whole feature.',
    tradeoffs: 'Two providers means two API keys and two bills, but each model works its strongest lane — Opus for prose and judgment, Sonnet for grounded code walkthroughs — and either provider can cover for the other during an outage.',
    mistakesWeMade: [
    ...
    stack: ['Next.js 14', 'Anthropic Claude Opus + Sonnet', 'OpenAI GPT-5.6', 'Tavily'],
