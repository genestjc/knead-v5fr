 ...
 * Query user's USDC balance and initiate client-side withdrawal.
 * 
  ...
  try {
    ...
    if (balance < amountNum) {
      return NextResponse.json({ 
        error: `Insufficient balance. You have $${balance.toFixed(2)}, but tried to withdraw $${amountNum.toFixed(2)}.` 
      }, { status: 400 });
    }
 ...
 * Get user's current USDC balance from blockchain
 */
