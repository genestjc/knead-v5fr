...
export async function POST(req: NextRequest) {
  try {
    ...
    const transaction = await burn({ contract, from: ownerAddress, tokenId: BigInt(tokenId), amount: 1n });
    
    const supabase = createSupabaseAdmin();
    await supabase.from('chat_users').update({ role: 'viewer', contributor_type: null }).eq('address', ownerAddress.toLowerCase());
