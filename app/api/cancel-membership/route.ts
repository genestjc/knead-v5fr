...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`Processing cancellation for ${user_address}, subscription ${subscriptionId}`);
    ...
    try {
      ...
      if (shouldBurnNow) {
        try {
          console.log("Burning NFT for immediate cancellation");
          ...
          if (balance > 0n) {
            console.log(`User ${user_address} has premium token, burning it`);
            
            const transaction = prepareContractCall({
              contract,
              method: "function adminBurn(address from, uint256 id, uint256 amount)",
              params: [user_address, BigInt(PREMIUM_TOKEN_ID), 1n],
            });
            ...
            console.log(`User ${user_address} doesn't have premium token, nothing to burn`);
          }
        } catch (burnError) {
      ...
      await supabase
        .from("subscriptions")
        .update({
          status: shouldBurnNow ? "cancelled" : "cancelling",
