...
export async function POST(req: NextRequest) {
  try {
    const { senderAddress, recipientAddress, productHandle, quantity = 1, shippingAddress, channelId } = await req.json();
    if (!senderAddress || !recipientAddress || !productHandle) return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
