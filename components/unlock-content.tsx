...
export function UnlockContent({ children, storySlug }: UnlockContentProps) {
  ...
  const checkAccess = async (walletAddress: string) => {
    ...
    try {
      ...
      const singleStoryContract = SINGLE_STORY_PASSES[storySlug]
      if (singleStoryContract) {
        console.log(`Checking single-story pass for ${storySlug}: ${singleStoryContract}`)
        ...
        try {
          ...
          if (balance > 0n) {
            ...
            setMembershipType("single-story")
            setCanAccess(true)
            setLoading(false)
            return
          }
        } catch (error) {
          console.error(`Error checking single-story pass for ${storySlug}:`, error)
        }
      }
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-lg max-w-md w-full mx-4">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowStripeSubscription(false)} className="text-gray-500 hover:text-gray-700">
              ×
            </button>
          </div>
          <StripeSubscription email={userEmail} user_address={account.address} onSuccess={handleSubscriptionSuccess} />
        </div>
      </div>
    )
  }
