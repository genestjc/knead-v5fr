...
export function AdminContextMenu({ message, eventId, position, onClose }: AdminContextMenuProps) {
  ...
  const handleBanUser = async () => {
    if (!confirm(`Ban ${message.sender.name}? They will not be able to send messages.`)) return;
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed z-50 bg-white rounded-lg shadow-2xl border border-gray-200 overflow-hidden min-w-[200px]"
        style={{
          left: position.x,
          top: position.y,
        }}
