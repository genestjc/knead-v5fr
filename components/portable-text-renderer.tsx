...
// --- All your original helper components are preserved ---
...
const InstagramEmbed = ({ value }: { value: { url: string; caption?: string } }) => {
    ...
    return <div className="my-8 animate-fade-in-up animation-delay-200"><div className="flex justify-center"><blockquote className="instagram-media" data-instgrm-permalink={`https://www.instagram.com/p/${postId}/`} data-instgrm-version="14" style={{ background: "#FFF", border: "0", borderRadius: "3px", boxShadow: "0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15)", margin: "1px", maxWidth: "540px", minWidth: "326px", padding: "0", width: "99.375%" }}><div style={{ padding: "16px" }}><a href={`https://www.instagram.com/p/${postId}/`} style={{ color: "#000", fontFamily: "Arial,sans-serif", fontSize: "14px", fontStyle: "normal", fontWeight: "normal", lineHeight: "17px", textDecoration: "none", wordWrap: "break-word" }} target="_blank">View this post on Instagram</a></div></blockquote></div></div>{value.caption && (<p className="mt-3 text-sm text-gray-600 text-center font-georgia-pro italic">{value.caption}</p>)}<script async src="//www.instagram.com/embed.js"></script></div>;
};
const TwitterEmbed = ({ value }: { value: { url: string; caption?: string } }) => {
    ...
    return <div className="my-8 animate-fade-in-up animation-delay-200"><div className="flex justify-center"><blockquote className="twitter-tweet" data-theme="light"><a href={value.url}></a></blockquote></div>{value.caption && (<p className="mt-3 text-sm text-gray-600 text-center font-georgia-pro italic">{value.caption}</p>)}<script async src="https://platform.twitter.com/widgets.js" charSet="utf-8"></script></div>;
};
...
const CustomImage = ({ value }: { value: any }) => {
  if (!value?.asset) return null; // Defensive check for image
...
const components = {
  // This `types` object is restored from your original code
  types: {
    image: CustomImage,
  ...
  // This `marks` object is restored from your original code
  marks: {
  ...
  // **THE FIX IS APPLIED HERE TO YOUR ORIGINAL `block` RENDERERS**
  block: {
    normal: ({ children }: { children?: React.ReactNode[] }) => {
      // If children is not a valid array, or is an array with just one empty string, render nothing.
  ...
  // This `list` object is restored from your original code
  list: {
